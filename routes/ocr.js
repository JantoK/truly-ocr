...
function extractInformation(data) {
    ...
    data.forEach(item => {
        ...
        if (!foundName && text.includes('姓名')) {
            info['姓名'] = text.replace('姓名', '').trim();
            foundName = true;
        } else if (!foundGenderAndEthnic && (text.includes('性别')||text.includes('民族'))) {
            const genderAndNation = text.replace('性别', '').trim();
            // 获取性别
            if(text.includes('女')||text.includes('男')){
                info['性别'] = text.includes('女')?'女':'男'
                info['民族'] = text.substring(text.includes(info['性别']) + 4).trim();
            } else {
                const genderIndex = genderAndNation.indexOf('民族');
                if (genderIndex !== -1) {
                    info['民族'] = genderAndNation.substring(genderIndex + 2).trim();
                }
            }
            foundGenderAndEthnic = true;
        } else if (!foundBirth && text.includes('出生')) {
            info['出生'] = text.replace('出生', '').trim();
            foundBirth = true;
...
router.post('/idCard', upload.single('imgFile'),async function(req, res, next) {
    try {
    // 图片文件
    ...
    ocr.flush({ image_base64: imgBuffer })
        .then((data) => {
